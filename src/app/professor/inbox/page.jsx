...
// //                                 {isPoll
// //                                   ? isVotable
// //                                     ? `Schedule Selection: ${data?.courseCode || "Unknown Course"}`
// //                                     : hasVoted
// //                                       ? `Schedule Selection: ${data?.courseCode || "Unknown Course"} (Voted)`
// //                                       : isScheduleSelectionPoll
// //                                         ? `Schedule Selection: ${data?.courseCode || "Unknown Course"}`
...
//                       voteCount: processedVotes[option._id] || 0,
//                     })) || [],
//                   course: notification.course ? notification.course.title : "",
...
//   // Function to extract required selections from reason text
//   const getRequiredSelections = (reason) => {
//     if (!reason) return 0
...
//       // Find the notification to get the required selections
//       const notification = notifications.find((n) => n.pollData?._id === pollId)
//       const requiredSelections = getRequiredSelections(notification?.pollData?.reason)
...
//                 const currentSelections = selectedOptions[pollId] || []
//                 const isVotingLoading = votingLoading[pollId] || false
//                 const requiredSelections = isPoll ? getRequiredSelections(notification.pollData?.reason) : 0
//                 const canSubmitVote = isVotable && currentSelections.length === requiredSelections
//                 console.log("data1234567890", data)
...
//                                   {isPoll
//                                     ? isVotable
//                                       ? `Schedule Selection${data?.courseCode ? `: ${data.courseCode}` : ""}`
//                                       : hasVoted
//                                         ? `Schedule Selection${data?.courseCode ? `: ${data.courseCode}` : ""} (Voted)`
//                                         : isScheduleSelectionPoll
//                                           ? `Schedule Selection${data?.courseCode ? `: ${data.courseCode}` : ""}`
//                                           : `Poll${data?.course ? `: ${data.course}` : ""}`
//                                     : data?.title || "No Title"}
//                                 </h3>
//                                 {!notification.isRead && !hasVoted && (
//                                   <div className="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0 mt-2"></div>
...
//                                   {/* Poll Options or Already Voted Message */}
//                                   {isVotable && notification.isRead ? (
//                                     <div className="text-sm text-gray-700 flex items-start gap-2 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
//                                       <HiInformationCircle className="w-5 h-5 flex-shrink-0 mt-0.5 text-yellow-600" />
//                                       <span className="text-yellow-800">
//                                         You have already selected the course schedule. For any changes, contact admin
//                                         via Gmail.
//                                       </span>
//                                     </div>
//                                   ) : (
...
//                                           />
//                                           {isVotable
//                                             ? `Select Your Preferred Options (${requiredSelections} required):`
//                                             : hasVoted
//                                               ? "Your Vote Has Been Submitted:"
//                                               : "Poll Statistics:"}
//                                         </h4>
...
//                                           <Button
//                                             onClick={() => submitVote(pollId, currentSelections)}
//                                             disabled={isVotingLoading || !canSubmitVote}
//                                             className="bg-orange-600 hover:bg-orange-700 text-white text-sm px-4 py-2 disabled:opacity-50 disabled:cursor-not-allowed"
//                                           >
//                                             {isVotingLoading ? (
//                                               <>
//                                                 <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
...
//                                               <>
//                                                 <HiCheck className="w-4 h-4 mr-2" />
//                                                 Submit Vote ({currentSelections.length}/{requiredSelections})
//                                               </>
//                                             )}
//                                           </Button>
//                                         )}
...
//                                               }`}
//                                               onClick={
//                                                 isVotable ? () => handleOptionSelect(pollId, option._id) : undefined
//                                               }
//                                             >
//                                               {/* Vote percentage bar */}
//                                               {!isVotable && (
...
//                                         <HiChartBar className="w-4 h-4 flex-shrink-0 mt-0.5 text-green-600" />
//                                         <span className="text-green-700">
//                                           This poll was sent to your students. View the current response statistics
//                                           above.
//                                         </span>
//                                       </div>
//                                     ) : isVotable ? (
...
//                                         <HiCheckCircle className="w-4 h-4 flex-shrink-0 mt-0.5 text-orange-600" />
//                                         <span className="text-orange-700">
//                                           Schedule selection poll - Select your preferred time slots and submit your
//                                           vote.
//                                         </span>
//                                       </div>
//                                     ) : hasVoted ? (
...
//                                         <HiCheck className="w-4 h-4 flex-shrink-0 mt-0.5 text-green-600" />
//                                         <span className="text-green-700">
//                                           You have already submitted your vote for this schedule selection poll. Thank
...
//                                     </div>
//                                   ) : hasVoted ? (
//                                     <div className="text-sm text-gray-700 flex items-start gap-2 p-4 bg-green-50 rounded-lg border border-green-200">
//                                       <HiCheck className="w-5 h-5 flex-shrink-0 mt-0.5 text-green-600" />
//                                       <span className="text-green-800">
//                                         You have already selected the course schedule and submitted your vote for this
//                                         schedule selection poll. Thank you for your response! For any changes, contact
//                                         admin via Gmail.
//                                       </span>
//                                     </div>
//                                   ) : (
...
//                                         </span>
//                                       </div>
//                                     ) : isScheduleSelectionPoll && !notification.isRead ? (
//                                       <div className="text-xs sm:text-sm text-gray-600 flex items-start gap-2 p-3 bg-orange-50 rounded-lg border border-orange-200">
//                                         <HiEye className="w-4 h-4 flex-shrink-0 mt-0.5 text-orange-600" />
//                                         <span className="text-orange-700">
//                                           Schedule selection poll from colleague. You can view results but cannot vote.
...
export default function ProfessorInboxPage() {
  ...
  const filterNotifications = () => {
    ...
    if (filterType !== "all") {
      if (filterType === "schedule_selection") {
        // Filter for schedule selection polls specifically
        filtered = filtered.filter(
          (notification) => notification.type === "poll" && notification.pollData?.pollType === "schedule selection",
        )
      } else {
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="poll">Polls</SelectItem>
                  <SelectItem value="message">Messages</SelectItem>
                  <SelectItem value="schedule_selection">Schedule Selection</SelectItem>
                </SelectContent>
              </Select>
              <Select value={filterRead} onValueChange={setFilterRead}>
                <SelectTrigger className="flex-1 min-w-32 sm:w-32 sm:flex-none bg-white/80 backdrop-blur-md border-gray-200 text-sm">
                              ...
                              >
                                ...
                                {isVotable && notification.isRead ? (
                                  <div className="text-sm text-gray-700 flex items-start gap-2 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                                    <HiInformationCircle className="w-5 h-5 flex-shrink-0 mt-0.5 text-yellow-600" />
                                    <span className="text-yellow-800">
                                      You have already selected the course schedule. For any changes, contact admin via
                                      Gmail.
                                    </span>
                                  </div>
                                ) : hasVoted ? (
